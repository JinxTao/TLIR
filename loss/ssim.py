...
class SSIM(torch.nn.Module):
    ...
    def forward(self, img1, img2):
        ...
        for i in [2,9,12]:
            f_fake = self.contentFunc(i).forward(img2)
            f_real = self.contentFunc(i).forward(img1)
            f_real_no_grad = f_real.detach()
            loss = self.loss(f_fake, f_real_no_grad)
            loss_avg += loss
        ploss = loss_avg/3

        return ssim_map*ploss
