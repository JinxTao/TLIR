...
if __name__ == "__main__":
    ...
    if opt['phase'] == 'train':
        while current_step < n_iter:
            ...
            for _, train_data in enumerate(train_loader):
                ...
                if current_step > n_iter:
                    break
                # np.save('/media/ext_disk/wangtao/save/duibi/result_model/1.npy',train_data['HI'])
                # np.save('/media/ext_disk/wangtao/save/duibi/result_model/2.npy',train_data['RI'])
                ...
                if current_step % opt['train']['val_freq'] == 0:
                    ...
                    for _,  val_data in enumerate(val_loader):
                        ...
                        li_img = Metrics.tensor2img(visuals['LI'])  # uint8
                        fake_img = Metrics.tensor2img(visuals['INF'])  # uint8
                        # generation
                        Metrics.save_img(
                            hi_img, '{}/{}_{}_hi.npy'.format(result_path, current_step, idx))
                        Metrics.save_img(
                            ri_img, '{}/{}_{}_ri.npy'.format(result_path, current_step, idx))
                        Metrics.save_img(
                            li_img, '{}/{}_{}_li.npy'.format(result_path, current_step, idx))
                        Metrics.save_img(
                            fake_img, '{}/{}_{}_inf.npy'.format(result_path, current_step, idx))
                        temp = np.transpose(np.concatenate(
                                (fake_img, ri_img, hi_img), axis=1), [2, 0, 1])
                        # tb_logger.add_image(
                        ...
                        if wandb_logger:
                            wandb_logger.log_image(
                                f'validation_{idx}', 
                                np.concatenate((fake_img, ri_img, hi_img), axis=1)
                            )
        ...
        for _,  val_data in enumerate(val_loader):
            ...
            # generation
            eval_psnr = Metrics.calculate_psnr(Metrics.tensor2img(visuals['RI'][-1]), hi_img)
            eval_ssim = Metrics.calculate_ssim(Metrics.tensor2img(visuals['RI'][-1]), hi_img)

            avg_psnr += eval_psnr
            avg_ssim += eval_ssim

            if wandb_logger and opt['log_eval']:
                wandb_logger.log_eval_data(fake_img, Metrics.tensor2img(visuals['RI'][-1]), hi_img, eval_psnr, eval_ssim)
